...
# I/O
out_dir = 'out'
model_output_name = 'ckpt.pt' # name of the model output file
eval_interval = 2000
log_interval = 1
eval_iters = 200
...
wandb_run_name = 'phop-16-4M-looped_2_0_6_random' # <base_block_size>_<loop_start>_<num_loops>.pt
# data
dataset = 'phop'
train_file_paths = [
...
def get_batch(split):
    # Reuse persistent DataLoader iterators to preserve prefetch pipeline
    ...
    # batch shape: (B, 2, BLOCK_SIZE)
    X = batch[:, 0, :].to(device, non_blocking=True).long()
...
elif init_from == 'scratch_loop':
    print("Initializing a new model from scratch")
    if meta_vocab_size is None:
        print("defaulting to vocab_size of GPT-2 to 50304 (50257 rounded up for efficiency)")
    model_args['vocab_size'] = meta_vocab_size if meta_vocab_size is not None else 50304
    gptconf = GPTConfig(**model_args)
    model = GPTLooped(gptconf, num_loops=num_loops, loop_start = loop_start, loop_func=loop_func)        
